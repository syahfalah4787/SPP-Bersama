...
<head>
    ...
    <title>Login Siswa - SPP Bersama</title>
    <link rel="preconnect" href="https://fonts.bunny.net">
    <link href="https://fonts.bunny.net/css?family=inter:400,600,700,800&display=swap" rel="stylesheet" />
    @vite(['resources/css/app.css', 'resources/js/app.js'])
    <style>
        .navy-blue { background-color: #0b213f; }
        .navy-blue-text { color: #0b213f; }
        /* Polygon to match the bottom left diagonal shape in the design */
        .clip-diagonal { clip-path: polygon(0 45%, 45% 100%, 0 100%); }
...
<body class="font-sans antialiased min-h-screen relative overflow-hidden bg-slate-100">
    ...
    <div class="absolute inset-0 z-30 flex items-center justify-center">
        <div class="navy-blue rounded-lg shadow-2xl w-[340px] h-[440px] flex flex-col relative overflow-hidden">
            ...
            <form method="POST" action="{{ route('siswa.login.submit') }}" class="px-8 mt-10 flex-1 flex flex-col">
                @csrf
                ...
                <div class="relative mb-5">
                    <input type="text" name="nisn" value="{{ old('nisn') }}" required placeholder="NISN"
                ...
                <div class="relative mb-8">
                    <input type="password" name="password" id="password-siswa" required placeholder="Password"
                           class="w-full h-9 px-3 pr-8 text-[13px] text-slate-800 bg-white border-none focus:ring-0 rounded-sm shadow-inner" />
                    <button type="button" onclick="togglePassword('password-siswa', 'eye-icon-siswa')" class="absolute inset-y-0 right-3 flex items-center text-slate-500 hover:text-slate-700">
                        <!-- Eye Slash Icon -->
                        <svg id="eye-icon-siswa" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"/></svg>
                    </button>
                </div>
            ...
            <!-- Login as Admin Link -->
            <a href="{{ route('login') }}" class="absolute bottom-5 left-6 text-[12px] italic text-[#4ea8de] hover:text-white transition">
